...
.button input{
    ...
    <div class="dash-content">
        <div class="overview">
            <div class="title">
                <span class="text">Guest Attendance </span>
            </div>
            <form action="guestattend.php" method="POST">
                <div class="field input">
                    <label>Id no</label>
                    <input type="text" name="idno" placeholder="Id No" required>
                </div>
                <div class="field button">
                    <input type="submit" name="search" value="View Guest">
                </div> 
            </form>
            <?php
            if(isset($_POST["search"])) {
                $idno = $_POST["idno"];
                $que = "SELECT * FROM guest WHERE idno='$idno'";
...
if(isset($_POST["attend"]))
{
    ...
    $checkColumnResult = mysqli_query($conn, $checkColumnQuery);
        ...
        $addColumnQuery = "ALTER TABLE `guest` ADD `$columnName` VARCHAR(255)";
    ...
    for($i = 0; $i < count($idnos); $i++)
    {
        ...
        $attendanceValue = $conn->real_escape_string($attendanceValues[$i]);
              $updateQuery = "UPDATE `guest` SET `$columnName` = '$attendanceValue' WHERE idno='$idno'";
